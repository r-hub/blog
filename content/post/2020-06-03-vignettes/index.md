...
:bulb: Based on what we said in the previous subsection, CMD build builds vignettes from `vignettes/` whereas R CMD check checks they can be rebuilt from `inst/doc/`. So if there were data in `vignettes/`, given it's not copied to `inst/doc/`... R CMD check will error!
