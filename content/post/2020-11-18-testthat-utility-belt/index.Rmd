...
Remember our post about [internal functions in R packages](/2019/12/12/internal-functions/)?
...
* R scripts under `tests/testthat/` whose name starts with `setup` are loaded before tests are run but not with `devtools::load_all()`. This can be important: it means the code in test setup files is _not_ available when you try debugging a test error (or developping a new test) by running `devtools::load_all()` then the code of your test.[^debug] And yes, you'll be interactively debugging tests more often than you wish. :wink:

[^debug]: If you use something like `browser()`, `debug()` etc. somewhere in your code and run the tests, the setup file will have been loaded.

* R scripts under `tests/testthat/` whose name start with `helper` are loaded with `devtools::load_all()`[^options] so they are available for both tests and interactive debugging. Just like... R scripts under `R/` so you might put your testthat helpers in the R directory instead, as recommended in testthat docs. So instead of living in `tests/testthat/helper.R` they'd live in e.g. `R/testthat-helpers.R` (the name is not important in the R directory). However, it also means they are installed with the package which might (slightly!) increase its size, and that they are with the rest of your package code which might [put you off](https://community.rstudio.com/t/why-are-tests-testthat-helper-files-discouraged-in-testthat/85253).

[^options]: Actually you can choose to have `devtools::load_all()` _not_ load the testthat helpers by using its `helpers` argument (`TRUE` by default).

To summarize,

| File                           | Run before tests | Loaded via `load_all()` | Installed with the package[^installed] | Testable[^testable] |
|--------------------------------|------------------|-------------------------|----------------------------|----------|
...
[^installed]: Installed with the package, and [run at package installation time](https://github.com/r-lib/testthat/issues/1206#issuecomment-713519962).
[^testable]: Yes, you could [test the code supporting your tests](https://github.com/r-lib/testthat/issues/1206#issuecomment-713583205).
...
You'll notice testthat no longer recommends having a file with code to be run after tests... So how do you clean up after tests?
Well, use [withr](https://withr.r-lib.org/index.html)'s various helper functions for deferring clean-up.
So basically it means the code for cleaning lives near the code for making a mess.
To learn more about this, read [the "self-cleaning text fixtures" vignette in testthat](https://testthat.r-lib.org/articles/test-fixtures.html) that includes examples.
...
So you'll need to (re-)read [the "self-cleaning text fixtures" vignette in testthat](https://testthat.r-lib.org/articles/test-fixtures.html).
