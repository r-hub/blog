...
* You could use `piggyback` to store versioned datasets in GitHub releases. The documentation of `piggyback` includes a thorough compariosn with other approaches such as git-LFS.

* You could use... write your own web API? Like the web APIs powering R-hub's own `pkgsearch`, `rversions`. The web APIs have value of their own, and you can write wrapper functions in your package.

That's all good but with non data packages a question is, where does the local data live once the user saved it, since it's no longer simply in a package?

* You could cache them in an app dir

* Or use another approach, e.g. the [`bowerbird` package](https://docs.ropensci.org/bowerbird/)
