...
To ship dependencies in your package, copy the dependencies into a sub-directory of `inst` in your package (but not `inst/src` or `inst/lib`, these are reserved directory names).
...
Some web dependencies only need to be included in the output document and don't require any HTML tags. In these cases, the dependency can appear alone in the `htmltools::tagList()`, as in [this example from xaringanExtra::use_tachyons()](https://github.com/gadenbuie/xaringanExtra/blob/master/R/tachyons.R). The names of these types of functions commonly include the `use_` prefix.
