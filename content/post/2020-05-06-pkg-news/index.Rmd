...
* There is an `usethis` shortcut for creating a NEWS.md: [`usethis::use_news_md()`](https://usethis.r-lib.org/reference/use_news_md.html); and using `usethis` to increase version will [add the corresponding heading to NEWS.md](https://usethis.r-lib.org/reference/use_version.html). Also note the interesting [`newsmd` package](https://github.com/Dschaykib/newsmd), like a `desc` for NEWS, that might help you write your own helpers?
...
res <- knitr::knit_child('errornews.Rmd', quiet = TRUE, envir = new.env())
cat(res, sep = '\n')
```
...
```{r, echo=FALSE, results='asis'}
res <- knitr::knit_child('errornews2.Rmd', quiet = TRUE, envir = new.env())
cat(res, sep = '\n')
```

* Another caveat, more specific to GitHub, is that NEWS.md locally don't render links to issues magically, unlike GitHub releases (and unlike `pkgdown`). That is why [Yihui Xie uses a placeholder inst/NEWS.Rd in e.g. `knitr`](https://github.com/yihui/knitr/issues/1841#issuecomment-621247442).
...
What is your own workflow for informing users?
