...
Mocking consists in modifying the behavior of a function *from outside*, replacing it with a mock (`how` in `mockery::stub()`) for a given context (`where` `mockery::stub()`).

For our previous example we would use:

```{r}
mockery::stub(
  where = is_internet_down,
  what = "my_complicated_code", 
  how = TRUE
)
is_internet_down()
my_complicated_code()
