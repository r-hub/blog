...
readmes_lengths <- dplyr::group_by(readmes, owner, repo) %>%
  dplyr::summarise(lines_no = count_lines(readme[[1]]))
```

The `readmes_lengths` data.frame has 114 lines so we lost a few more packages. How long are usual READMEs?
...
To access the numbers we're after, we don't need too much regular expressions of m
