...
Intestingly there is still no app involved but httr starts a web server using httpuv. 
...
* With httr you can directly pass the token to `httr::VERB()` via the `config` parameter. httr also automatically refreshed your access token if the token object contains a valid refresh token.
...
Now if you are storing your HTTP interactions to disk e.g. if using the [vcr package for HTTP testing](https://books.ropensci.org/http-testing/vcr.html), you will need to know how the secrets are used in HTTP request.
Often,
...
Please share any further tip or example you like in the comments below!
